...
def pre_load_data(run_numbers, prefix, p):
    ...
    for rn in run_numbers:
        ...
        if p['wavelength'] > 0:
            LoadDNSLegacy(os.path.join(p['path'], infile), Normalization='no', ElasticChannel=p['e_channel'], Wavelength=p['wavelength'], OutputWorkspace=wsname)
        else:
            LoadDNSLegacy(os.path.join(p['path'], infile), Normalization='no', ElasticChannel=p['e_channel'], OutputWorkspace=wsname)
